...
class HumanIdleBehaviors {
    constructor(page) {
        ...
        this.idleActions = [
            ...
            // zoomActivity USUNIETE: Ctrl+scroll zoom jest anomalia na FB (audit #10)
    ...
    async tabActivity() {
        if (Math.random() < 0.1) { // 10% szans na akcję tab
            const actions = [
                ...
                // F5 USUNIETE: odswieza strone i przerywa skanowanie (audit #10)
    ...
    /**
     * ETAP 6.4: Symulacja zmiany taba (visibilitychange)
     * Facebook nasluchuje tego eventu - konto ktore NIGDY nie traci focusu jest podejrzane.
     */
    async tabSwitch() {
        try {
            // Ukryj strone (uzytkownik "przeszedl na inny tab")
            ...
            await setTimeout(5000 + Math.random() * 25000);

            // Wroc na strone (uzytkownik "wraca na tab z Facebookiem")
            await this.page.evaluate(() => {
