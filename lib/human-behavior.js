...
const HUMAN_DELAYS = {
    ...
    afterPageLoad: { min: 2000, max: 6000 },       // 2-6s po załadowaniu strony
...
async function humanMouseMove(page, targetX, targetY) {
    // Ciaglosc ruchu: startuj z ostatniej znanej pozycji myszy na tej stronie.
    // Przy pierwszym ruchu na nowej stronie - srodek okna przegladarki.
    // page.viewport() zwraca null przy defaultViewport: null, dlatego uzywamy window.innerWidth.
    let startX = page._lastMouseX;
    let startY = page._lastMouseY;
    ...
    // Zapamietaj koncowa pozycje dla ciaglosci nastepnego ruchu
...
/**
 * ETAP 6.3: Micro-interakcje z trescia
 * Symuluje naturalne zachowanie: hover na reakcjach, avatar autora.
 * Nigdy nie klika - tylko najedza mysza i odjedza.
 */
async function simulateContentEngagement(page, postElement) {
    // 10% szans: najedz mysza na przycisk reakcji (hover bez klikniecia)
    if (Math.random() < 0.10) {
        try {
            ...
            if (likeButton) {
                ...
                if (box) {
                    ...
                    await sleep(800 + Math.random() * 1500); // hover 0.8-2.3s
                    // Odjedz mysza bez klikania
                    const awayX = box.x + 100, awayY = box.y + 50;
                    await page.mouse.move(awayX, awayY);
                    page._lastMouseX = awayX;
                    page._lastMouseY = awayY;
                }
            }
        } catch (e) { /* ignoruj */ }
    }
        ...
        try {
            ...
            if (avatar) {
                ...
                if (box) {
                    ...
                    const awayX = box.x - 50, awayY = box.y + 30;
                    await page.mouse.move(awayX, awayY);
                    page._lastMouseX = awayX;
                    page._lastMouseY = awayY;
                }
...
/**
 * ETAP 8.1: Obsluga checkpointu - reaguj jak czlowiek
 * @param {object} page - Puppeteer page
 * @param {function} alertCallback - callback do wysylania alertow
 * @returns {object} { shouldStop, cooldownHours, requiresHumanAction }
 */
async function handleCheckpoint(page, alertCallback) {
    ...
    await setTimeout(30000 + Math.random() * 30000); // 30-60 sekund
    ...
    try {
        ...
        const screenshotPath = `${logsDir}/checkpoint-${Date.now()}.png`;
        await page.screenshot({ path: screenshotPath, fullPage: true });
        console.log(`📸 Screenshot checkpointu: ${screenshotPath}`);
    } catch (e) {
        console.log('⚠️ Nie udalo sie zrobic screenshota checkpointu');
    }
    ...
    if (alertCallback) {
        try {
            ...
            console.log('⚠️ Nie udalo sie wyslac alertu:', e.message);
        }
    }
