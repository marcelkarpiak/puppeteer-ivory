...
class DistributedCoordinator {
    ...
    async distributeTask(task) {
        ...
        // Odśwież lastSeen bieżącej instancji, żeby nie wypadła podczas przerw między grupami
        if (this.instances.has(this.instanceId)) {
            this.instances.get(this.instanceId).lastSeen = Date.now();
        } else {
            await this.registerInstance();
        }
    ...
    async executeScrapingTask(task) {
        ...
        const { runSingleSession } = require('../fb-bot');
